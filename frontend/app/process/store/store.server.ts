...
// import { getCacheBackend } from "#app/process/store";
import type { ICacheBackend } from "#app/process/store";
import { getSettings, getSettingsSession } from "#app/shared/api/settings";
  ...
  public async get(request: Request, uuid?: string) {
    try {
      ...
      if (!settingsData || !settings) {
        settingsData = await this.loadAndRevalidate(
          request,
          settings.uuid ?? uuid
        );
      }
