...
export const Error: FC<TProps> = ({ message, caught, error }) => {
  ...
  const errorMessage =
    message || error?.message || t("errorBoundary.common.unexpectedError");
  ...
  if (isDev && caught?.data) {
    try {
      ...
      formattedCaughtData = JSON.stringify(
        parsedData,
        undefined,
              ...
              <>
                ...
                {caught && (
                  <div className="Error-Caught">
                    ...
                    <pre className="Error-CaughtFormatted">
                      Data: {formattedCaughtData ?? "-"}
                    </pre>
                  </div>
                )}
