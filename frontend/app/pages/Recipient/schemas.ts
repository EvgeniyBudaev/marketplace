...
import {
  ...
  EMAIL_REGEXP,
  EMPTY_FIELD_ERROR_MESSAGE, NAME_ERROR_MESSAGE, NAME_REGEXP, PHONE_ERROR_MESSAGE, PHONE_REGEXP,
} from "~/shared/validation";

export const formSchema = z
  .object({
    [EFormFields.Name]: z.string().trim().min(1, EMPTY_FIELD_ERROR_MESSAGE).regex(NAME_REGEXP, NAME_ERROR_MESSAGE),
    [EFormFields.Surname]: z.string().trim().min(1, EMPTY_FIELD_ERROR_MESSAGE).regex(NAME_REGEXP, NAME_ERROR_MESSAGE),
