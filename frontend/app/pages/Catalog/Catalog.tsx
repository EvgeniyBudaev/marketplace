...
import { attributeItemLinks } from "#app/pages/Catalog/AttributeItem";
import {
  filterLinks,
  getDefaultFilter,
  Filter,
} from "#app/pages/Catalog/Filter";
import { panelLinks, Panel } from "#app/pages/Catalog/Panel";
import { productListLinks, ProductList } from "#app/pages/Catalog/ProductList";
import { productListItemLinks } from "#app/pages/Catalog/ProductListItem";
import { sortingLinks } from "#app/pages/Catalog/Sorting";
...
import type { TParams, TSorting } from "#app/types";
...
export const Catalog: FC<TProps> = (props) => {
  ...
  useEffect(() => {
    if (lastLoadedPage.current !== page || isFiltersDirty) {
      ...
      if (pages[page - 1] === undefined || isFiltersDirty) {
        setSearchParams(new URLSearchParams(getFormData() as any), {
  ...
  const getRenderedProductsCount = useCallback(() => {
    return pages
      .map((page) => page?.length ?? DEFAULT_PAGE_SIZE)
      .reduce((a, b) => a + b, 0);
  }, [pages]);
    ...
    <section className="Catalog">
      <div className="Catalog-Row">
        ...
        <Typography variant={ETypographyVariant.TextB3Regular}>
          {productRange.endProduct} {t("pages.catalogs.from")}{" "}
          {products.countOfResult} {t("pages.catalogs.goods")}
        </Typography>
      </div>
          ...
          >
            <ProductList
              ...
              pages={pages.map(
                (page) =>
                  page ??
                  Array(products.pageSize ?? DEFAULT_PAGE_SIZE).fill(
                    PLACEHOLDER_PRODUCT
                  )
              )}
              isCardsLine={isCardsLine}
