...
import {
  ...
  mapFormDataToDto,
  useGetTypeOptions,
} from "#app/pages/Admin/Attributes/AttributeEdit";
import type {
  TAddModalState,
  TForm,
  TOptionsSubmitForm,
} from "#app/pages/Admin/Attributes/AttributeEdit";
import { SelectableAddModal } from "#app/pages/Admin/Attributes/SelectableAddModal";
import { SelectableTable } from "#app/pages/Admin/Attributes/SelectableTable";
import {
  EAttributeAction,
...
export const AttributeEdit: FC<TProps> = (props) => {
  ...
  const attribute: TAttributeDetail =
    fetcherRemix.data?.attribute ?? props.attribute;
  ...
  const [filter, setFilter] = useState<TParams>({
    filter: [attribute.filter ? idCheckbox : []],
  });
  ...
  const handleSubmitEditSelectableValue = ({
    id,
    value,
  }: {
    id: number;
      ...
      >
        ...
        {isSelectableType && (
          <>
            <div className="AttributeEdit-FormFieldGroup">
              <Tooltip
                message={t("pages.admin.attributeEdit.tooltip.buttonAdd")}
              >
                <Button
