...
import { Error as ErrorComponent } from "#app/components";
import type {
  TCatchBoundaryMessage,
  TCatchBoundaryStatusMap,
  TWithChildrenProps,
} from "./types";
...
export const createCatchBoundary = ({
  ...
  return () => {
    ...
    const statusMapItem =
      isRouteErrorResponse(caught) && statusMap?.get(caught.status);
    const statusMapMessage =
      typeof statusMapItem !== "function" ? statusMapItem : statusMapItem(t);
    ...
    const formattedDefaultStatusMessage =
      typeof defaultStatusMessage !== "function"
        ? defaultStatusMessage
        : defaultStatusMessage(t);
