...
import { DEFAULT_PAGE_SIZE, DEFAULT_PAGE_SIZE_LIST } from "#app/constants";
...
} from "#app/uikit/components/Table/NavigationPanel";
import {
  TableBody,
  tableBodyLinks,
} from "#app/uikit/components/Table/TableBody";
import { optionsLinks } from "#app/uikit/components/Table/Options";
import {
  TableHeader,
  tableHeaderLinks,
} from "#app/uikit/components/Table/TableHeader";
import {
  TableLoader,
  tableLoaderLinks,
} from "#app/uikit/components/Table/TableLoader";
...
const TableComponent = <TColumn extends Record<string, any>>(
  ...
  const updateSpinnerPosition = useCallback(() => {
    ...
    const visibleTop = Math.max(
      0,
      Math.min(window.innerHeight, boundingRect.y)
    );
    const visibleBottom = Math.max(
      0,
      Math.min(window.innerHeight, boundingRect.bottom)
    );
  ...
  useEffect(() => {
    ...
    function handleScroll() {
      ...
      wrapperRef.current.style.maxHeight = `${
        document.documentElement.clientHeight - bbox.top
      }px`;
    }
    ...
    <div data-testid={dataTestId} ref={ref}>
      {hasData && pagesCount && (
        <NavigationPanel
          ...
          defaultPageSize={
            !isNil(defaultPageSize) ? defaultPageSize : DEFAULT_PAGE_SIZE
          }
          ...
          pageSizeOptions={
            !isNil(pageSizeOptions) ? pageSizeOptions : DEFAULT_PAGE_SIZE_LIST
          }
          theme={theme}
        />
      )}
      <div className="Table-Head">
        ...
        <div>
          {settings && (
            <Control {...settings} columns={table.getAllLeafColumns()} />
          )}
        </div>
      </div>
