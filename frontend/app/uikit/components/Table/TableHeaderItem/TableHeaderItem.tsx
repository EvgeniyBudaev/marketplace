...
import type { TTableSortingHandleChange } from "#app/uikit";
...
type TProps<T extends object> = {
  ...
  onChange: (
    value:
      | TSortingColumnStateWithReset
      | Array<TSortingColumnStateWithReset>
      | null
  ) => void;
...
export const TableHeaderItem = <T extends object>({
  ...
  const sortingState = multiple
    ? (state as Array<TSortingColumnStateWithReset>).find(
        (item) => item.sortProperty === headerId
      )
  ...
  const handleChange = ({
    ...
    if (
      sortingState &&
      sortProperty === sortingState.sortProperty &&
      shouldReset
    ) {
      if (multiple) {
        onChange(
  ...
  const renderPopoverContent = (close: any) => {
    return (
      <ul className="TableHeaderItem-DropDownList">
        ...
        <li
          className="TableHeaderItem-DropDownListItem"
          onClick={() => handleHideColumn(close)}
        >
          <div className="TableHeaderItem-DropDownListItem-Icon">
