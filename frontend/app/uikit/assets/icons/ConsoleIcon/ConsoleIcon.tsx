import { memo } from "react";
import type { FC } from "react";
import type { TIconProps } from "../types";

const IconComponent: FC<TIconProps> = ({ className, onClick, ...props }) => (
  <svg
    className={className}
    version="1.1"
    xmlns="http://www.w3.org/2000/svg"
    x="0px"
    y="0px"
    viewBox="0 0 330.98 330.98"
    onClick={onClick}
    {...props}
  >
    <g>
      <path
        d="M319.98,199.43h-71.575V91.043c0-45.722-37.195-82.918-82.916-82.918c-45.719,0-82.914,37.196-82.914,82.918V199.43H11
		c-6.075,0-11,4.925-11,11v112.426h22v-42.152h286.98v42.152h22V210.43C330.98,204.354,326.056,199.43,319.98,199.43z
		 M104.396,91.043c0-33.69,27.406-61.098,61.094-61.098c33.689,0,61.096,27.408,61.096,61.098v108.082H104.396V91.043z
		 M157.49,264.703H22V221.43h135.49V264.703z M308.98,264.703H173.49V221.43h135.49V264.703z"
      />
      <path
        d="M185.558,89.294c-1.239-1.102-3.135-0.987-4.234,0.252l-43.5,49c-1.101,1.238-0.987,3.135,0.252,4.234
		c0.571,0.508,1.282,0.757,1.99,0.757c0.827,0,1.651-0.341,2.244-1.009l43.5-49C186.91,92.29,186.797,90.393,185.558,89.294z"
      />
      <path
        d="M188.671,105.323l-29.043,32.715c-1.101,1.238-0.987,3.135,0.252,4.234c0.571,0.508,1.282,0.757,1.99,0.757
		c0.827,0,1.651-0.341,2.244-1.009l29.043-32.715c1.101-1.238,0.987-3.135-0.252-4.234
		C191.666,103.97,189.77,104.084,188.671,105.323z"
      />
      <path
        d="M80.445,241.348h22.668c3.313,0,6-2.686,6-6c0-3.313-2.687-6-6-6H80.445c-3.313,0-6,2.687-6,6
		C74.445,238.661,77.132,241.348,80.445,241.348z"
      />
      <path
        d="M227.867,241.348h22.668c3.314,0,6-2.686,6-6c0-3.313-2.686-6-6-6h-22.668c-3.314,0-6,2.687-6,6
		C221.867,238.661,224.554,241.348,227.867,241.348z"
      />
    </g>
  </svg>
);

export const ConsoleIcon = memo(IconComponent);
