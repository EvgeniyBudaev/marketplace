...
import { JWTService, TJWTServiceParams } from "./JWTService.ts";
import { wrapDataFunction } from "./wrapDataFunction.ts";
...
export function registerAccessTokenRefresh(
  ...
  const routes: ServerRouteManifest = Object.entries(build.routes).reduce(
    (acc, [routeId, route]) => {
      if (
        !routeIdPrefixList ||
        routeIdPrefixList.every((item) => !routeId.startsWith(item))
      ) {
        return {
          ...acc,
          [routeId]: route,
      ...
      const action =
        route.module.action &&
        wrapDataFunction(route.module.action, jwtService);
      const loader =
        route.module.loader &&
        wrapDataFunction(route.module.loader, jwtService);
      const newRoute = {
        ...route,
        module: {
          ...route.module,
